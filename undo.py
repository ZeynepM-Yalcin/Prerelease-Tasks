...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        OriginalGrid = list(self.__Grid)#copy of current grid
        OriginalSymbolsLeft = self.__SymbolsLeft#copy f symbols left
        OriginalScore = self.__Score#copy of score
        ...
        while not Finished:
            ...
            OriginalCell = OriginalGrid[(self.__GridSize - Row) * self.__GridSize + Column - 1]

            Undo = input("Do you want to undo your last move? (y/n) ").lower()
            if Undo == "y":#
                self.__Grid = list(OriginalGrid)
                self.__SymbolsLeft = OriginalSymbolsLeft
                self.__Score = OriginalScore - 3
                CurrentCell.UpdateCell()
                print("Move undone. Your score is now: " + str(self.__Score))
                continue#
