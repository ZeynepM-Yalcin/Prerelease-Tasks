...
class Puzzle():
                ...
                for Count in range (1, self.__GridSize * self.__GridSize + 1):
                    ...
                    if Items[0] == "@":
                        ...
                        for CurrentSymbol in range(1, len(Items)):
                            #print(CurrentSymbol)
                            #self.CheckForMatchWithPattern(Current)
        ...
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    ...
                    # Check for the presence of another pattern in the same 3x3 grid
                    #OtherPattern = self.CheckforPatternInGrid(StartRow, StartColumn, Row, Column)
                    #if OtherPattern:
                        # Double the score if there are two patterns in the 3x3 grid
                        #self.__Score *= 2
    ...
    def CheckForNoPattern(self, match):
        size = self.__GridSize
        noSymbol = 0
        for StartRow in range(size, 0, -1): #loop through grid
            for StartColumn in range(1,size+1):
                if self.GetSymbol() != "-":
                    noSymbol += 1
        valid = match * 3
        nvalid = noSymbol - valid
        print(nvalid)
        return nvalid
                

   
    def ResetBlocks(self): #to reset all blocked cells
        size = self.__GridSize
        for StartRow in range(size, 0, -1): #loop through grid
            for StartColumn in range(1,size+1):
                self.__GetCell(StartRow, StartColumn).AddToNotAllowedSymbols(0) #set zero to reset array
    

    def LoopGrid(self):   #loop through grid to reset blocks
    
        score = 0       # score is reset each time
        size = self.__GridSize
...
class Cell():
    ...
    def AddToNotAllowedSymbols(self, SymbolToAdd):
        if SymbolToAdd == 0:     #symbol is 0 so reset array
            self.__SymbolsNotAllowed = []
        else:
            self.__SymbolsNotAllowed.append(SymbolToAdd) #add symbol to array
